    ...
    html.Div(className='navbar-fixed',children=[
        html.Nav([
            html.Div(className='nav-wrapper grey darken-4',children=[
                dcc.Link(className='brand-logo left',children=[html.I(className='material-icons left',children=['blur_on']),'Real-Time Dashboard'],href='/daily_layout')
            ])
        ])
    ]),
    html.Br(),
...
def update_output(n_clicks,start_date,end_date,n,lt_oi,gt_oi,lt_price,gt_price,sector,scrip):
    if n_clicks:
        ...
        OI_hist_mc_merged['Price Change %'] = OI_hist_mc_merged.groupby('Scrip')['Close'].apply(lambda x: x.pct_change())
        OI_hist_mc_merged['Price Change %'] = OI_hist_mc_merged['Price Change %'].apply(lambda x: x*100)
        OI_hist_mc_merged['OI Change %'] = OI_hist_mc_merged.groupby('Scrip')['Cumulative OI'].apply(lambda x: x.pct_change())
        ...
        OI_hist_mc_filtered = OI_hist_mc_merged.loc[np.logical_and(OI_hist_mc_merged['Date'] >= start_date, OI_hist_mc_merged['Date'] <= end_date),:]
        OI_hist_mc_filtered = OI_hist_mc_filtered.dropna()
        OI_hist_mc_filtered.loc[:,'OI Percentile'] = OI_hist_mc_filtered.groupby('Scrip')['OI Change %'].rank(pct=True).mul(100).round(2)
        OI_hist_mc_filtered.loc[:,'Price Percentile'] = OI_hist_mc_filtered.groupby('Scrip', group_keys=False)['Price Change %'].rank(pct=True).mul(100).round(2)
...
if __name__ == '__main__':
    app.run_server(host='localhost',port=9600)
